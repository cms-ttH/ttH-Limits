...
for i in range(fpf_s.getSize()):
    ...
    if nuis_p == None:
        ...
        mean_p = nuis_p.getVal()
        sigma_p_err = nuis_p.getError()

        hiErr = nuis_p.getAsymErrorHi()
        loErr = nuis_p.getAsymErrorLo()
        if nuis_p.hasRange("err68"):
            hiErr = nuis_p.getMax("err68") - mean_p
            loErr = nuis_p.getMax("err68") - mean_p

        maxError = max( max(hiErr, loErr), sigma_p_err )

        if hiErr<0.001*maxError :
            hiErr = sigma_p_err

        if loErr<0.001*maxError :
            loErr = sigma_p_err

        sigma_p = 0.5 * ( hiErr + loErr )
    ...
    for fit_name, nuis_x in [('b', nuis_b), ('s',nuis_s)]:
        if nuis_x == None:
            ...
            nuis_x_val = nuis_x.getVal()
            nuis_x_err = nuis_x.getError()

            hiErr = nuis_x.getAsymErrorHi()
            loErr = nuis_x.getAsymErrorLo()
            if nuis_x.hasRange("err68"):
                hiErr = nuis_x.getMax("err68") - nuis_x_val
                loErr = nuis_x.getMax("err68") - nuis_x_val

            hiErr = abs(hiErr)
            loErr = abs(loErr)
            
            maxError = max( max(hiErr, loErr), nuis_x_err )
            ...
            nuis_x_err = 0.5 * ( hiErr + loErr )

            #if name=="CMS_scale_j":
            #    print "\t CMS_scale_j,\t val = %.3f,\t err = %.3f,\t asymErr = %.3f,\t errLo = %.3f,\t errHi = %.3f\n " % ( nuis_x_val, nuis_x.getError(), nuis_x_err, nuis_x.getAsymErrorLo(), nuis_x.getAsymErrorHi() )
 
            row += [ "%+.2f +/- %.2f" % (nuis_x.getVal(), nuis_x_err) ]
            if nuis_p != None:
                valShift = (nuis_x.getVal() - mean_p)/sigma_p
                if fit_name == 'b':
                    pulls.append(valShift)
                sigShift = nuis_x_err/sigma_p
                if options.abs:
