...
def write_datacard(file, discriminant, categories, cats, samples, systematics,
        limited_systematics={}, ofile=log, is8TeV=True):
    """
    """
...
shapes ttH_hbb * {f} ttH$MASS_hbb_{d}_$CHANNEL ttH$MASS_hbb_{d}_$CHANNEL_$SYSTEMATIC
shapes ttH_hcc * {f} ttH$MASS_hcc_{d}_$CHANNEL ttH$MASS_hcc_{d}_$CHANNEL_$SYSTEMATIC
shapes ttH_hww * {f} ttH$MASS_hww_{d}_$CHANNEL ttH$MASS_hww_{d}_$CHANNEL_$SYSTEMATIC
    ...
    for (unc, type, vals) in systematics:
        ...
        for (n, s) in samples:
            ...
            if "ttH" in s:
                ...
                #s_new = s + "_M125"
                s_new = s_new.replace("ttH","ttH125")
            #log.write(" is = {ss}\n".format(ss=s_new))
            file_s = s_new
            #log.write(" file_s = {s}\n".format(s=file_s))
            for c in cats[s]:
...
def create_datacard(ifile, ofile, disc, all_categories,
    ...
    active_unc = write_datacard(ifile, disc, categories, cats, samples,
            systematics, limited_systematics, ofile=ofile, is8TeV=is_8_tev)
